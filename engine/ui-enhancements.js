(function (global) {
  ...
  function setPlanetDetail(panel, direction) {
    ...
    const destination = mapData[mapId].directions[direction];
    const destinationMap = destination.mapId
      ? mapData[destination.mapId]
      : null;
    ...
    if (!destinationMap) {
      detail.innerHTML = `
        ...
        <div>
          ...
          <h3>Territoire non cartographié</h3>
          <p><b>Biome :</b> Données indisponibles.</p>
          <p><b>Ressources :</b> Aucune observation enregistrée.</p>
          <p><b>Point de vue de BlueFox :</b> Je ne peux pas décrire cette direction avant de l’avoir atteinte en jeu.</p>
        </div>`;
      return;
    }
    detail.innerHTML = `
  ...
  function renderCatalogMaps(panel) {
    ...
    heading.textContent = "BIOMES CATALOGUÉS";
    future.appendChild(heading);
    if (!maps.length) {
      const empty = document.createElement("p");
      empty.textContent =
        "Les nouvelles scènes du dossier Images apparaîtront ici après génération du catalogue.";
      future.appendChild(empty);
      return;
    }
  ...
  function enhancePlanet(panel) {
    ...
    mapGrid.querySelectorAll("button").forEach((button) => {
      ...
      const target = mapData[current].directions[direction].mapId;
      ...
      content.querySelector("small b").textContent =
        known ? mapData[target].name : "Non explorée";
    ...
    if (!panel.querySelector(".planet-future-space")) {
      ...
      future.innerHTML = "<span>EXTENSIONS PLANÉTAIRES</span><p>Emplacement réservé aux futures cartes, routes mémorisées et synthèses d’exploration.</p>";
